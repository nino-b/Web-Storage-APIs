...
const Menu = {
    ...
    load: async () => {
        ...
        if (Menu.data) {
            ...
            Menu.data.forEach(category => {
                imageCache.addAll(category.products.map(p => `../data/images/${p.image}`));
            });
        }
    },
    loadCacheFirst: async () => {
        // Cache First pattern
        const db = await Menu.openDB();
        /* count will count the amount of objects or values in one data store */
        if (await db.count('categories') === 0) {
            const categories = await API.fetchMenu(); // if necessary, stringify it
